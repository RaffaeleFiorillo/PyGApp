...
def load_image(directory: str) -> Surface:
    try:
        ...
        exit(f"Error: Directory *assets/images/{directory}* Not Found")
...
# Creates button images based on the provided strings that represent the button's labels (for both on and off buttons)
def get_label_images(labels: [str]) -> ([Surface], [Surface]):
    label_images_active, label_images_passive = [], []
    ...
    for label in labels:
        for text_size in range(40)[::-1]:
            ...
            text_active = text_font.render(label, True, (0, 0, 0))
            text_passive = text_font.render(label, True, (255, 255, 255))
        ...
        label_images_active.append(active)
        label_images_passive.append(passive)
    return label_images_active, label_images_passive
